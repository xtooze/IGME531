...
WriteSVG(new DrawFunctions(), "../../../TileTestNoise.svg");
...
void WriteSVG(DrawFunctions df, string path)
{
    ...
    try
    {
        ...
        sw.WriteLine(drawTile(df,4,4,4));
...
string drawTile(DrawFunctions df, double tileWidth, double tileHeight, int boundMax)
{
    string tileGrid = "";
...
string drawTileNoise(DrawFunctions df, double tileWidth, double tileHeight, int boundMax, float[,] noiseGrid)
{
    string tileGrid = "";
    for (double i = 0; i < 100; i += tileWidth)
    {
